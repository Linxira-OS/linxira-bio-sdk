...
description: Trim and preprocess local FASTQ reads with Linxira Bio executable capabilities. Use for plain, gzip, or BGZF FASTQ 3' quality trimming, minimum-length filtering, sequencing adapter removal, and producing a new FASTQ artifact with `fastq.trim.v1` or `fastq.adapter.v1`.
---
...
2. Report read counts, discarded reads, trimmed reads, input/output bases, and
   quality- or adapter-trimmed bases.
...
- Adapter matching is exact and 3'-oriented in this capability version; it does
  not perform error-tolerant matching, paired-end synchronization, UMI parsing,
  poly-G/poly-X trimming, or duplicate removal.
