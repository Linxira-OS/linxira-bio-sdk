...
| MCP server                           | `linxira-bio-mcp` crate **已实现**：stdio JSON-RPC（initialize/tools/list/tools/call→worker 执行/resources list+read，protocolVersion 2024-11-05）；但 **未进发布包**，README 仍标 "MCP server: planned"                                                                                                                           | [mcp/main.rs](engine/crates/linxira-bio-mcp/src/main.rs)；[bundle-manifest.json](packaging/bundle-manifest.json) `binaries` 仅 CLI/worker/UI 三个                                              |
...
| L3 | **MCP server**（[linxira-bio-mcp](engine/crates/linxira-bio-mcp/src/main.rs)，stdio JSON-RPC） | crate 已实现但未发布、未注册客户端 | 面向 Claude Desktop / Cursor 等 MCP 客户端 |
